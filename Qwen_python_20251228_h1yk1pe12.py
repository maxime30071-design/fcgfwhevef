...
async def receive_requirements(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_update:
        # Сохраняем токен в .env только при создании
        with open(os.path.join(bot_dir, ".env"), "w") as f:
            f.write(f"BOT_TOKEN={context.user_data['bot_token']}")
    await update.message.reply_text("Все файлы получены! " + ("Обновляю бота..." if is_update else "Запускаю бота..."))
    if is_update:
        stop_bot(bot_dir)
    success = setup_and_run_bot(bot_dir, context.user_data.get("bot_token") if not is_update else None)
    if success:
        await update.message.reply_text("🟢 Бот успешно " + ("обновлён" if is_update else "запущен") + "! Проверь его в Telegram.")
    else:
        await update.message.reply_text("🔴 Ошибка при " + ("обновлении" if is_update else "запуске") + ". Проверь логи.")
    return ConversationHandler.END
def setup_and_run_bot(bot_dir: str, bot_token: str = None) -> bool:
    ...
    try:
        ...
        with open(os.path.join(bot_dir, "pid.txt"), "w") as f:
...
async def update_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not os.path.exists(bot_dir):
        await update.message.reply_text("Сначала создайте бота через /start.")
        return ConversationHandler.END
    context.user_data.clear()
    context.user_data['is_update'] = True
    await update.message.reply_text("Отправьте новый файл `bot.py`:")
    return BOT_CODE
...
if __name__ == "__main__":
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("start", start), CommandHandler("update", update_bot)],
        states={
